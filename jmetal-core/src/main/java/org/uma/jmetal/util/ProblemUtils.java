...
public class ProblemUtils {
  ...
  public static <S extends Solution<?>> Problem<S> loadProblem(String problemName) {
    ...
    @SuppressWarnings("unchecked")
    public static <S extends Solution<?>> Problem<S> loadProblem(String problemName,Object[] a) {
    Problem<S> problem ;
    try {
      if(a.length==2)
      problem = (Problem<S>)Class.forName(problemName).getConstructor(Integer.class,Integer.class).newInstance(a[0],a[1]);
      else if(a.length==3)
      problem = (Problem<S>)Class.forName(problemName).getConstructor(Integer.class,Integer.class,Integer.class).newInstance(a[0],a[1],a[2]);    
      else
      problem=null;
    } catch (InstantiationException e) {
      throw new JMetalException("newInstance() cannot instantiate (abstract class)", e) ;
