...
import org.uma.jmetal.qualityindicator.impl.hypervolume.WFGHypervolumeV2;
...
public class MicroFAME<S extends Solution<?>> extends SteadyStateNSGAII<S> {
   ...
   * Constructor
   */
  public MicroFAME(Problem<S> problem, int maxEvaluations, int populationSize,
    ...
    private void load_fuzzy_motor()
    {
        ...
        T1MF_Gauangle lowStagnationUMF = new T1MF_Gauangle("Upper MF for low Stagnation",-0.4, 0.0, 0.4);
        T1MF_Gauangle midStagnationUMF = new T1MF_Gauangle("Upper MF for mid Stagnation",0.1, 0.5, 0.9);
        T1MF_Gauangle highStagnationUMF = new T1MF_Gauangle("Upper MF for high Stagnation",0.6, 1.0, 1.4);

        T1MF_Gauangle lowOperatoruseUMF = new T1MF_Gauangle("Upper MF for low Operatoruse",-0.4, 0.0, 0.4);
        T1MF_Gauangle midOperatoruseUMF = new T1MF_Gauangle("Upper MF for mid Operatoruse",0.1, 0.5, 0.9);
        T1MF_Gauangle highOperatoruseUMF = new T1MF_Gauangle("Upper MF for highOperatoruse",0.6, 1.0, 1.4);

                
        T1MF_Gauangle lowProbabilityUMF = new T1MF_Gauangle("Upper MF for low Probability",-0.4, 0.0, 0.4);
        T1MF_Gauangle midProbabilityUMF = new T1MF_Gauangle("Upper MF for mid Probability",0.1, 0.5, 0.9);
        T1MF_Gauangle highProbabilityUMF = new T1MF_Gauangle("Upper MF for high Probability",0.6, 1.0, 1.4);
        ...
        T1_Antecedent highStagnation=new T1_Antecedent("HighStagnation",highStagnationUMF,Stagnation);
        
        T1_Antecedent lowOperatoruse=new T1_Antecedent("LowStagnation",lowOperatoruseUMF,Stagnation);
        T1_Antecedent midOperatoruse=new T1_Antecedent("MidStagnation",midOperatoruseUMF,Stagnation);
        T1_Antecedent highOperatoruse=new T1_Antecedent("HighStagnation",highOperatoruseUMF,Stagnation);
        
        T1_Consequent lowProbability=new T1_Consequent("lowProbability",lowProbabilityUMF,Probability);
        T1_Consequent midProbability=new T1_Consequent("MidProbability",midProbabilityUMF,Probability);
        T1_Consequent highProbability=new T1_Consequent("HighProbability",highProbabilityUMF,Probability);
        
        
        rulebase = new T1_Rulebase(9);
        rulebase.addRule(new T1_Rule(new T1_Antecedent[]{highStagnation, highOperatoruse}, midProbability));
        rulebase.addRule(new T1_Rule(new T1_Antecedent[]{highStagnation, midOperatoruse}, lowProbability));
        rulebase.addRule(new T1_Rule(new T1_Antecedent[]{highStagnation, lowOperatoruse}, midProbability));
        rulebase.addRule(new T1_Rule(new T1_Antecedent[]{midStagnation, highOperatoruse}, midProbability));
        rulebase.addRule(new T1_Rule(new T1_Antecedent[]{midStagnation, midOperatoruse}, lowProbability));
        rulebase.addRule(new T1_Rule(new T1_Antecedent[]{midStagnation, lowOperatoruse}, midProbability));
        rulebase.addRule(new T1_Rule(new T1_Antecedent[]{lowStagnation, highOperatoruse}, highProbability));
        rulebase.addRule(new T1_Rule(new T1_Antecedent[]{lowStagnation, midOperatoruse}, midProbability));
        rulebase.addRule(new T1_Rule(new T1_Antecedent[]{lowStagnation, lowOperatoruse}, lowProbability));
        //print out the rules
        System.out.println("\n"+rulebase);
    ...
    while(counter>0)
          {
              ...
              {
                  operator++;
                  if(operator== operators_desirability.length)
                      operator=0;
              }
  ...
  @Override protected List<S> createInitialPopulation() {
    ...
    protected List<S> NewCrowdingDistanceSelection(Ranking<S> ranking) {
